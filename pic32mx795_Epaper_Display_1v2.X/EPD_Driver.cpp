...
void spi_init(void) {
    ...
    /* ---- 1. Deshabilitar SPI4 antes de reconfigurar ---- */
    SPI4CONbits.ON = 0;

    /* ---- 2. Configurar TRIS de pines SPI4 NATIVOS ----
     *
     * En el PIC32MX795F512H/L los pines del SPI4 son FIJOS (no usan PPS):
     *   SCK4 = RB14  (pin físico 29)  ? salida
     *   SDO4 = RF5   (pin físico 32)  ? salida
     *   SDI4 = RF4   (pin físico 31)  ? entrada
     *
     * NOTA IMPORTANTE: La familia PIC32MX5XX/6XX/7XX NO tiene los registros
     * CFGCONbits.IOLOCK, SDI4R ni RPF5R ? esos son de la familia 1XX/2XX/3XX.
     * Los pines SPI4 en el 795 son nativos y solo requieren configurar TRIS.
     */

    /* RB14 = SCK4 (salida nativa) */
    TRISBCLR = (1u << 14);

    /* RF5 = SDO4 (salida nativa) */
    TRISFCLR = (1u << 5);

    /* RF4 = SDI4 (entrada nativa) */
    TRISFSET = (1u << 4);

    /* RB3 = CS manual (GPIO salida) ? pin 35 en esquema interno */
    TRISBCLR = (1u << 3);
    LATBSET  = (1u << 3);   /* CS = HIGH (inactivo) */

    /* ---- 3. Configurar periférico SPI4 ---- */
    SPI4CON  = 0;           /* Reset completo */
    SPI4STAT = 0;

    /*
     * Baud Rate = Fpb / (2 * (SPIxBRG + 1))
     * Con Fpb = 80 MHz y objetivo 8 MHz:
     * SPIxBRG = (80 / (2*8)) - 1 = 5 - 1 = 4
     */
    SPI4BRG = 4;
    ...
    SPI4CONbits.CKP    = 0; /* Clock idle = LOW  (CPOL=0) */
    /*
     * BUG #4 CORREGIDO: CKE=1 para Pervasive Displays
     * CKE=1 ? datos cambian en flanco de BAJADA, se capturan en SUBIDA
     * Esto corresponde a SPI Mode 0 (CPOL=0, CPHA=0) según la hoja de datos
     * del controlador UC8171 del EPD.
     */
    SPI4CONbits.CKE    = 1;
    SPI4CONbits.SMP    = 0; /* Muestrear en el medio del dato */

    /* Limpiar overflow flag */
