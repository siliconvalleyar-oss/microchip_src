 ...
 * @brief   Configuraci√≥n completa: espejos globales y de fuente
 * 
...
void epd_drawChar(uint8_t* buffer, uint16_t x, uint16_t y, char c,
    ...
    uint16_t fontIndex = c * 8;   // √çndice directo por ASCII   // 8 columnas por car√°cter
    ...
    for (uint8_t col = 0; col < 8; col++) {
        ...
        for (uint8_t row = 0; row < 8; row++) {
            // Determinar la fila real despu√©s de posible volteo vertical
            uint8_t actualRow = row;
#if FONT_FLIP_VERTICAL
            actualRow = 7 - row;   // invertir verticalmente el car√°cter
#endif
            uint8_t bit;
#if FONT_MSB_FIRST
...
// Dibuja un car·cter de la fuente Terminal16x24e (16x24 pÌxeles)
void epd_drawChar16x24(uint8_t* buffer, uint16_t x, uint16_t y, char c,
                       uint8_t color, uint8_t bgColor) {
    if (c < 32 || c > 255) return;
    uint8_t index = c - 32;   // La fuente empieza en ASCII 32 (espacio)
    
    // La fuente tiene 48 bytes por car·cter: 16 columnas ◊ (24 filas / 8) = 16◊3
    for (uint8_t col = 0; col < 16; col++) {
        for (uint8_t row = 0; row < 24; row++) {
            // Cada columna ocupa 3 bytes (para las 24 filas)
            uint8_t byteIndex = col * 3 + (row / 8);
