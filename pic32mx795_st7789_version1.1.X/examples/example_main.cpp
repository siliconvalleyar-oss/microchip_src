...
/*
 * ST7789 Driver + Space Shooter Avanzado
 ...
 *  - Colisiones, vidas, puntuación
 *  - Control por botones (RB7=izq, RB8=der, RB9=disparo)
 *  - Namespaces y gestión estática de memoria
...
// ======================== PRESENTACIÓN (descomentar para habilitar) ========================
#define PRESENTACION   // Comentar esta línea para omitir la presentación
...
// Botones (pull-up interno, activos a nivel bajo)
#define BTN_LEFT_PIN  7   // RB7
#define BTN_RIGHT_PIN 8   // RB8
#define BTN_FIRE_PIN  9   // RB9
...
// ======================== NAMESPACE GRÁFICO ========================
namespace Graphics {
    // Fuente 5x7 (95 caracteres imprimibles)
...
void Game::spawnEnemy(void) {
    for(int i=0; i<MAX_ENEMIES; i++) {
        if(!enemies[i].active) {
            ...
            enemies[i].color = (rand() % 2) ? RED : COLOR565(255,100,100);
...
void Game::updateEnemies(void) {
    for(int i=0; i<MAX_ENEMIES; i++) {
        if(enemies[i].active) {
            Graphics::fill_rect(enemies[i].x, enemies[i].y, enemies[i].w, enemies[i].h, BLACK);
            ...
            if(enemies[i].y > TFT_H) {
                ...
                Graphics::fill_rect(enemies[i].x, enemies[i].y, enemies[i].w, enemies[i].h, enemies[i].color);
            }
        }
    }
...
void Game::updateShip(void) {
    ...
    if(BTN_LEFT) {
        if(!leftPressed) { shipX -= 8; leftPressed=1; }
    } else leftPressed=0;
    if(BTN_RIGHT) {
        if(!rightPressed) { shipX += 8; rightPressed=1; }
    } else rightPressed=0;
    ...
    if(shipX != prevShipX || shipY != prevShipY) {
        if(prevShipX != -1 && prevShipY != -1) {
            Graphics::erase_ship(prevShipX, prevShipY, BLACK);
        }
