...
// Configuración de bits del PIC32MX795
...
// ============================================================
//  HARDWARE — Delay, SPI4, Display ST7789
// ============================================================
...
void spi4_init(void) {
    DDPCON  = 0x00;
    AD1PCFG = 0xFFFF;
    TRISBCLR = (1<<6);   // RB6 = SCK4
    TRISFCLR = (1<<5);   // RF5 = SDO4
    TRISFSET = (1<<4);   // RF4 = SDI4
    SYSKEY = 0xAA996655;
    SYSKEY = 0x556699AA;
    OSCCONCLR = (1<<6);
    ...
    *RPOR3  = (*RPOR3  & 0xFFFFFF00) | 3;
    *RPOR2  = (*RPOR2  & 0x0000FFFF) | (4<<16);
...
void init_display(void) {
    ...
    write_cmd(0xE0);
    const uint8_t gp[]={0xD0,0x04,0x0D,0x11,0x13,0x2B,0x3F,
                        0x54,0x4C,0x18,0x0D,0x0B,0x1F,0x23};
    for(int i=0;i<14;i++) write_data(gp[i]);
    write_cmd(0xE1);
    const uint8_t gn[]={0xD0,0x04,0x0C,0x11,0x13,0x2C,0x3F,
...
// ============================================================
int main(void) {
    // Pines de pantalla como salidas
    BL_TRIS=0; DC_TRIS=0; RST_TRIS=0;
    DC_LAT=0; RST_LAT=1; BL_LAT=0;

    spi4_init();
    init_display();
    BL_LAT = 1;   // Encender backlight

    srand(_CP0_GET_COUNT());
    sound_init();

    // Arrancar el motor de juego (incluye pantalla de título)
